...
trait HasRoleAndPermission
{
    ...
    {
        return $this->belongsToMany(config('dacl.models.role'))->withTimestamps()->withPivot('granted');
    }
    ...
    public function __call($method, $parameters)
    {
        if (Str::startsWith($method, 'roleIs')) {
            return $this->roleIs(Str::snake_case(substr($method, 6), config('roles.separator')));
        } elseif (Str::startsWith($method, 'may')) {
            return $this->may(Str::snake_case(substr($method, 3), config('roles.separator')));
        } elseif (Str::startsWith($method, 'allowed')) {
            return $this->allowed(Str::snake_case(substr($method, 7), config('roles.separator')), $parameters[0], (isset($parameters[1])) ? $parameters[1] : true, (isset($parameters[2])) ? $parameters[2] : 'user_id');
        } elseif (Str::startsWith($method, 'hasRole')) {
            return $this->hasRole(isset($parameters[0]) ? $parameters[0] : Str::snake_case(substr($method, 7), config('roles.separator')));
        }
